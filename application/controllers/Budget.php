...
public function index()
    ...
    public function getProduitByactivite_json()
    {
        ...
        //$codeProjet = $idactivite ;
        
        
        $budgets = $this->budget->getByactivite($idprojet);
...
public function donner_details()
{
    ...
    ", false)
    ->from('budget b')
    ->join("(SELECT am.idbudget, SUM(l.montant_depense) AS total_depense, SUM(l.montant_return) AS total_return_liquidation
            FROM avance_mission am JOIN liquidation l ON l.idmission = am.idmission GROUP BY am.idbudget) ld",
           'ld.idbudget = b.idbudget', 'left')
    ->join("(SELECT am.idbudget, SUM(r.montantReturn) AS total_return_relique
