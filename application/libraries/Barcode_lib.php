...
class Barcode_lib {
    ...
    public function generate($filepath = "", $text = "0", $size = "20", $orientation = "horizontal", $code_type = "code128", $print = false, $sizefactor = "1") {
        $code_string = "";
        ...
        if (in_array(strtolower($code_type), array("code128", "code128b"))) {
            ...
            $code_array2 = array("3-1-1-1-3", "1-3-1-1-3", "3-3-1-1-1", "1-1-3-1-3", "3-1-3-1-1", "1-3-3-1-1",
            ...
            for ($X = 1; $X <= strlen($text); $X += 2) {
                if (isset($temp[$X]) && isset($temp[($X + 1)])) {
                    ...
                    $code_string .= implode("-", $temp1) . "1";
                }
            }
            $code_string = "111-" . $code_string . "-111";
        } else {
            return false;
        }

        $width = (strlen($code_string) * 2) + 2;
        $height = $size;
        ...
        if (strtolower($orientation) == "horizontal") {
            for ($i = 0; $i < strlen($code_string); $i++) {
                if (substr($code_string, $i, 1) == "1") {
                    imageline($image, $x, 0, $x, $height, $foreground);
                }
                $x++;
            }
        } else {
            for ($i = 0; $i < strlen($code_string); $i++) {
                if (substr($code_string, $i, 1) == "1") {
                    imageline($image, 0, $i, $width, $i, $foreground);
                }
                $x++;
            }
        }
        ...
        if ($print) {
            $text = $text . " ";
            $font = 5;
            imagestring($image, $font, 0, $height - imagefontheight($font), $text, $foreground);
        }
