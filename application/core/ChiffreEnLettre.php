...
{
		public function getString($number = 10, $money = "Ariary", $centimes = "") {
		...
		private function corrections($string) {
			...
			if(strpos($string, 'et un mille') != false)
			{
				$text = explode('et un mille', $string);
				$string = "";
				$txt = "";

				foreach ($text as $key => $tx) 
				{
					...
					{
						$txt = $txt . str_replace("un mille", "mille", $tx);
					}
