			...
			.done(function (recherche_produit) {
				if (recherche_produit.success) {
					$.ajax({
						...
						data: {
							idmateriel: recherche_produit.materiel.idmateriel,
							id_pv: id_pv,
						},
						dataType: "json",
					})
							...
							if (response.success) {
								...
								if (quantite > 0) {
									...
									if (qte_produit_panier_demande[materiel.refmateriel]) {
										in_panier =
											qte_produit_panier_demande[materiel.refmateriel];
									}
									...
									Myalert.erreur(
										"Vous n'avez pas encore approvisionné ce materiel dans ce point de vente.",
									);
									vider();
								}
							} else {
...
function addToPanierTab(
	...
	if (
		...
		tableau_panier_demande[reference].length != 0
	) {
		...
		if (!teste_unite) {
			qte_produit_panier_demande[reference] =
				tableau_panier_demande[reference][0].qte_min;
			let data = {
				count: texte,
...
$(document).on("click", ".delete", function () {
	...
	$(document).on("click", "#confirmeDelete", function () {
		...
		if (tableau_panier_demande[identification[0]]) {
			...
			if (allunite_panier_demande[reference]) {
				let qte_deleted = qteMinUnit(
					allunite_panier_demande[reference],
					quantite,
				...
				qte_produit_panier_demande[reference] =
					parseInt(qte_produit_panier_demande[reference]) -
					parseInt(qte_deleted);
	...
	$(document.body).on("click", "#valider", function () {
		...
		const qte_dispo =
			quantite_dispo_tab[quantite_dispo_tab.length - 1].quantite;
		...
		if (
			tableau_panier_demande[reference] &&
			tableau_panier_demande[reference].length
		) {
			// additionner le quantiter dans le pannier et celle qui vien d'etre ajouter
			total_qte =
				parseInt(total_qte) +
				parseInt(tableau_panier_demande[reference][0].qte_min);
		}
		if (
			reference != "" &&
			idmateriel != "" &&
			...
			if (qte_dispo >= total_qte) {
				...
				addToPanierTab(
					idmateriel,
					qteMinUnit(real_unite, quantite, identification),
					quantite,
					reference,
					designationmateriel,
					description,
...
/**
 * validation du panier - OUVERTURE DU MODAL
 */
$(document).on("click", "#validerPanier", function () {
	// Récupérer l'agent sélectionné sur la page principale
	const idagent = $("#idagent").val();
	const agentNameHtml = $("#selected_agent_name").html(); // Contient le nom et les infos formatés

	let nom_beneficiaire = "";
	let tel_beneficiaire = "";

	if (idagent && idagent !== "") {
		// Extraire le nom complet du HTML (on prend juste la partie <strong>)
		const tempDiv = $("<div>").html(agentNameHtml);
		const strongText = tempDiv.find("strong").text();
		nom_beneficiaire = strongText;
		tel_beneficiaire = idagent;
	}

	// Générer le contenu du modal AVEC l'agent pré-rempli
	$("#validation").html(
		panier_modal_content(nom_beneficiaire, tel_beneficiaire),
	);
 ...
 */
/**
 * Contenu du modal de validation (SIMPLIFIÉ - sans recherche d'agent)
 * @param {*} nom_beneficiaire
 * @param {*} tel_beneficiaire
 * @returns
 */
function panier_modal_content(nom_beneficiaire = "", tel_beneficiaire = "") {
	...
	// En-tête du modal (affichage de l'agent)
	let entete = "";

	if (tel_beneficiaire === "" && nom_beneficiaire === "") {
		// Aucun agent sélectionné sur la page principale
		entete += `
        <div class="alert alert-warning mt-2">
            <i class="fa-solid fa-triangle-exclamation"></i> 
            <strong>Aucun agent sélectionné.</strong><br>
            Veuillez d'abord sélectionner un agent bénéficiaire dans le formulaire principal.
        </div>
        `;
	} else {
		// Agent sélectionné : affichage en lecture seule + champ caché pour l'ID
		entete += `
        <div id='beneficiaire_set' class="mt-2">
            <label class="form-label fw-bold">Agent bénéficiaire :</label>
            <div class="input-group">
                <input type="text" class="form-control" readonly value="${nom_beneficiaire}">
                <input type="hidden" id="idagent_modal" name="idagent_modal" value="${tel_beneficiaire}">
            </div>
            <small class="text-success"><i class="fa-solid fa-check-circle"></i> Agent validé depuis le formulaire principal</small>
        </div>
        `;
	}

	// Ajout du champ caché pour le point de vente
	entete += `<input id="id_pointdevente" type="hidden" class="form-control" value='${$("#pv_vente").val()}'>`;
		...
		$.ajax({
			...
			.done(function (response) {
				if (response.success == true) {
					...
					$("#validation").html(
						panier_modal_content(nom.toUpperCase() + " " + prenom, numero),
					);
					$(".modal-footer").removeClass("d-none");
				} else {
					Myalert.delete(
...
$(document).on("click", "#recherche_user", function () {
	$("#validation").html(
		recherche_user_content($("#beneficiaire_search").val()),
	);
});
/**
 * choix user
...
/**
 * ENVOI DU PANIER (Validation finale)
 */
$(document.body).on("click", "#sendvalidation", function () {
	// Récupérer l'ID agent depuis le champ caché du modal
	let iduser = $("#idagent_modal").val();

	// Vérification : si pas d'ID, on essaie de le récupérer du champ principal (fallback)
	if (!iduser || iduser === "") {
		iduser = $("#idagent").val();
	}

	if (iduser && iduser !== "" && iduser > 0) {
		...
		$.ajax({
			...
			// Envoyer les données
			$.ajax({
				url: base_url("Demande/validate"),
				type: "post",
				data: { data: data },
			})
				.done(function (data) {
					tableau_panier_demande = {};
					$("#validerPanier").click(); // Fermer le modal
					$("#validation").html("");
					$("#tableau").html(append_tableau());

					// Réinitialiser aussi l'affichage de l'agent sélectionné
					$("#idagent").val("");
				...
				.fail(function () {
					console.error("Erreur sur la validation de la demande !!");
