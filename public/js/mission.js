...
function select_budget(idactivite = '', idbudget = []) {
    ...
    $.ajax({
        ...
        type: "post",
        data: { idactivite: idactivite,idprojet:idprojet },
        dataType: 'json',
        async: false
        ...
        if (response.success) {
            ...
            for (let i = 0; i < datas.length; i++) {
                ...
                recherche += `
							<tr >
								<td> 
									<input class='choix_budget form-check-input' data-reste ='${element.reste}' data-ligne ='${element.ligne_budget}' data-check='${idbudget.includes(element.idbudget) ? true : false}' type="checkbox" name='budget' data-id='${element.idbudget}' data-designation='${element.designation_budget}' data-montant='${element.budget}' ${idbudget.includes(element.idbudget) ? 'checked' : ''}>
								</td>
                                <td>${element.ligne_budget}</td>
            ...
            recherche += `	
						...
						<button data-id='' class="btn btn-info btn-sm close_budget" data-bs-dismiss="modal" id="budget_valide">VALIDER</button>`  ;
            content = recherche;
        } else {
...
function check_budgete() {
    ...
    for (let i = 0; i < checks.length; i++) {
        ...
        if ($(element).attr('data-check') == 'true') {
            ...
            let reste = parseInt($(element).attr("data-reste"));
            let ligne = $(element).data('ligne');
            somme += montant;
            content += `
    ...
    console.log("Clic sur projet → ID =", idprojet);   // ← AJOUTE ÇA

    // effet visuel
    $('.filtre-projet').removeClass('projet_active');
    $(this).addClass('projet_active');
    console.log("URL appelée :", base_url + 'Mission/ajaxFiltreProjet');  // ← IMPORTANT

    $.ajax({
url: base_url('Mission/ajaxFiltreProjet'),        type: 'GET',
        ...
        success: function (html) {
            ...
            console.log("Réponse reçue :", html);   // ← AJOUTE ÇA
        },
        error: function (xhr, status, error) {
            console.error("Erreur AJAX :", status, error);   // ← TRÈS IMPORTANT
            $('#tbody-mission').html('<tr><td colspan="11">Erreur lors du chargement</td></tr>');
        }
    });
});
...
$(document).on('input change', '.all-avance', function () {
    ...
    $('.all-avance').each(function () {
        const val = parseFloat($(this).val().replace(',', '.')); // virgule → point
        if (!isNaN(val)) total += val;
    });

    // 1) stocke le total dans l'input caché déjà existant
    $('#avance').val(total);

    /*// 2) (facultatif) l'affiche quelque part pour l'utilisateur
    $('#total_avance').text(total.toLocaleString('fr-FR'));*/
});
