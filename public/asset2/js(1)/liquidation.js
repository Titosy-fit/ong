...
function depense_content(i = 0, j = 0) {
    let content = `<div class="row">
                        <div class="col">
                            <label class="form-label">Designation dépense</label>
                        ...
                        <div class="col">
                            <label class="form-label">Montant dépense : </label>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    let content = `<label class="form-label">Budget : </label>
                        ...
                        <div class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="accordion-button container_entete p-0 text-darck" type="button" data-bs-toggle="collapse"  data-bs-target="#collapse${i}" aria-expanded="true" aria-controls="collapse${i}">
                                    <div class="entete_accord text-darck p-2 buget_designation">${element.designation_budget}</div>
                                    <div class="entete_accord text-darck p-2 budget_montant">Total budget : ${parseFloat(element.budget).toLocaleString("fr-FR") + ' Ar'}</div>
                                    <input type="text" name="" class="input_like_text entete_accord text-darck p-2 " id='total_depenser_${i}' value="Total dépensé : 0 Ar" readonly > 
                                    <input type="number" class="d-none total_depense" id='total_depenser_val_${i}' value="0" readonly > 
                                </button>
                            </h2>
                    ...
                    if (response.type == 'liquide') {
                        Myalert.erreur('La mission est déjà dans la liste des liquidations.')
                    } else {
